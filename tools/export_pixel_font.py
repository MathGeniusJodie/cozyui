...
def decode_simple_glyph_units(data):
    ...
    if contour_count == 0:
        return [], (0, 0, 0, 0)
    if contour_count < 0:
        # Composite glyphs aren't supported by this exporter; the caller
        # skips the glyph (and prints a warning) instead of aborting the
        # whole export.
        return None
...
def decode_simple_glyph(data, units_per_pixel):
    decoded = decode_simple_glyph_units(data)
    if decoded is None:
        return None
    contours, bbox = decoded
    return [
...
def infer_units_per_pixel(font, cmap, glyf, offsets, codepoints):
    ...
    for codepoint in codepoints:
        ...
        if start == end:
            continue
        decoded = decode_simple_glyph_units(glyf[start:end])
        if decoded is None:
            continue
        contours, bbox = decoded
...
def main():
    ...
    for code in codepoints:
        ...
        contours = []
        if start != end:
            decoded = decode_simple_glyph(glyf[start:end], units_per_pixel)
            if decoded is None:
                print(
                    f"warning: skipping composite glyph for codepoint U+{code:04X} "
                    "(composite glyphs are not supported by this exporter)"
                )
                continue
            contours, bbox = decoded
    ...
    distinct_units = sorted({source["units_per_pixel"] for source in sources})
    if len(distinct_units) == 1:
        units_comment = f"// units_per_pixel={distinct_units[0]}"
    else:
        per_source = ", ".join(
            f"{font_path.name}={source['units_per_pixel']}"
            for font_path, source in zip(font_paths, sources)
        )
        units_comment = f"// units_per_pixel varies by source: {per_source}"
    atlas_entries = "\n".join(
        f"    FontAtlas {{\n"
    ...
    metrics_path.write_text(
        "\n".join(
            [
                ...
                f"{units_comment}, range=U+{first:04X}..U+{last:04X}",
